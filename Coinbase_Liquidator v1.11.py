...
# ## Release Notes ##
#
# v1.11 - Added comprehensive logging system with LOG_OUTPUT parameter and Liquidations folder for full console logs
# v1.10 - Enhanced display to show EXCLUDED_PAIRS assets with "Excluded" label for complete portfolio visibility
# v1.09 - Added SHOW_IGNORED parameter to display ignored assets with enhanced formatting
# v1.08 - Added value filtering with IGNORE_MIN_VALUE and IGNORE_MAX_VALUE parameters
...
# v1.02 - Replaced manual active_pairs with automated trading pair detection
...
DRY_RUN_MODE = True                     # Set to True to simulate without placing orders
...
IGNORE_MIN_VALUE = 0                     # Skip assets with value below this USD amount (0 to disable)
IGNORE_MAX_VALUE = 0                     # Skip assets with value above this USD amount (0 to disable)

# File configuration
PRODUCTS_FILE = "products.json"          # File to store Coinbase product information
PRODUCTS_MAX_AGE_HOURS = 72              # Hours before refreshing products file
...
def log(message):
    ...
    # Add to file log if enabled
    log_to_file(message)

def log_to_file(message):
    """Add message to log buffer for file output"""
    global log_buffer
    
    if LOG_OUTPUT and log_file_path:
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        log_entry = f"[{timestamp}] {message}"
        log_buffer.append(log_entry)
        
        # Write buffer if getting full (immediate write for liquidator)
...
def write_log_buffer():
    ...
    if not LOG_OUTPUT or not log_file_path or not log_buffer:
        return
...
def initialize_logging():
    """Initialize the logging system with timestamped filename"""
    global log_file_path
    
    if LOG_OUTPUT:
        # Create Liquidations directory if it doesn't exist
        logs_dir = os.path.join(SCRIPT_DIR, "Liquidations")
        os.makedirs(logs_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        log_file_path = os.path.join(logs_dir, f"Liquidations - {timestamp}.log")
        
        try:
            # Create log file with header
            with open(log_file_path, 'w', encoding='utf-8') as f:
                f.write(f"Coinbase Liquidator v1.11 Log\n")
                f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Configuration: DRY_RUN={DRY_RUN_MODE}, AUTO_CONFIRM={AUTO_CONFIRM_LIQUIDATION}\n")
                f.write(f"Excluded Assets: {EXCLUDED_PAIRS}\n")
                f.write("=" * 80 + "\n\n")
            debug_log(f"Logging initialized: {log_file_path}")
        except Exception as e:
...
def liquidate_portfolio():
    ...
    for asset, balance_info in balances.items():
        ...
        # Check if asset is excluded first
        if asset in EXCLUDED_PAIRS:
            if SHOW_IGNORED:
                # Calculate what would be sellable for display purposes
                available_balance = balance_info['available_balance']
                if base_increment and available_balance >= Decimal(str(base_increment)):
                    sellable_amount = (available_balance // Decimal(str(base_increment))) * Decimal(str(base_increment))
                else:
                    sellable_amount = available_balance
                log(f"✗ {asset:8} | Excluded: {sellable_amount:>15} | Value: ${balance_info['fiat_value']:>10.2f} | Pair: {pair_id}")
            elif SHOW_UNSELLABLE_ASSETS:
                log(f"✗ {asset:8} | Balance: {balance_info['available_balance']:>15} | Value: ${balance_info['fiat_value']:>8.2f} | Skip: Excluded asset")
            continue
        ...
        if can_liquidate:
            ...
            skip_reason = None
            if IGNORE_MIN_VALUE > 0 and fiat_value < IGNORE_MIN_VALUE:
                skip_reason = f"Value ${fiat_value:.2f} below minimum ${IGNORE_MIN_VALUE}"
            elif IGNORE_MAX_VALUE > 0 and fiat_value > IGNORE_MAX_VALUE:
                skip_reason = f"Value ${fiat_value:.2f} above maximum ${IGNORE_MAX_VALUE}"
            
            if skip_reason:
                if SHOW_IGNORED:
                    log(f"✗ {asset:8} | Ignored: {sellable_amount:>16} | Value: ${fiat_value:>10.2f} | Pair: {pair_id}")
                elif SHOW_UNSELLABLE_ASSETS:
                    log(f"✗ {asset:8} | Sellable: {sellable_amount:>15} | Value: ${fiat_value:>8.2f} | Skip: {skip_reason}")
            else:
                ...
                log(f"✓ {asset:8} | Sellable: {sellable_amount:>15} | Value: ${fiat_value:>8.2f} | Pair: {pair_id}")
        else:
            reason = result
